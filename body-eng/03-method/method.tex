...
\begin{umlfigure}[Implementation: Sequence diagram: Create 3D avatar from a portrait image.]
    ...
    Gateway -> Backend: Sends image
    Backend -> Backend: Processes image
    Backend -> Gateway: Sends 3D avatar
    Gateway -> Frontend: Sends 3D avatar
    Frontend -> User: Outputs 3D avatar
    User -> User: Views 3D avatar
    User -> Frontend: Downloads 3D avatar
    Frontend -> Gateway: Sends download request
    Gateway -> Backend: Sends download request
    Backend -> Backend: Sends 3D avatar zip file
    Backend -> Gateway: Sends 3D avatar zip file
    Gateway -> Frontend: Sends 3D avatar zip file
    Frontend -> User: Downloads 3D avatar zip file
    @enduml
\end{umlfigure}
