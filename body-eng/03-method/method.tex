...
%sequence diagram
\begin{umlfigure}[Sequence diagram: Create 3D avatar from a portrait image][seq_create_3d_avatar]
    @startuml
    actor User
    participant "3D Avatar Reconstruction System" as System
    User -> System: Upload image
    System -> System: Extract face region
    System -> System: Extract face texture
    System -> System: Extract hair region
    System -> System: Extract hair texture
    System -> System: Extract emotion
    System -> System: Reconstruct 3D head
    System -> System: Reconstruct 3D hair
    System -> System: Align head and hair
    System -> System: Output 3D avatar
    System -> User: Download 3D avatar
    @enduml
...
This use case is an extension of the main use case. The user can choose to try on different hairstyles with the 3D avatar. The system will then output a 3D avatar with the selected hairstyle. The user can choose to download the 3D avatar as a zip file, which contains the 3D avatar model and texture map.
