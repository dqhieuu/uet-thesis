...
For taking a single-view portrait image as input, the system provides a front-end interface, i.e. a web page, where the user can upload their image. The front-end interface allows the user to sends the image to a gateway server, which is responsible for handling the user's requests and sending the image to the back-end server for processing. The back-end server is responsible for processing the image and sending the 3D avatar to the user. The system's architecture is shown in the figure below.
