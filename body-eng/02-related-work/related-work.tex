...
MICA \cite{zielonkaMetricalReconstructionHuman2022} is a method to reconstruct 3D face models from a single-view image, which also uses FLAME as a component in the process of reconstructing the 3D model. However, unlike DECA, MICA only reconstructs the 3D face model with the shape parameters ($\beta$) and does not reconstruct the expression parameters as well as the texture. Which also means that MICA does not reconstruct the facial expression.

MICA uses a different approach to reconstruct the shape parameters to DECA. MICA bases its network on the ArcFace architecture \cite{dengSubcenterArcFaceBoosting2020}
